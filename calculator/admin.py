...
class SaleAdmin(admin.ModelAdmin):
    list_display = ['project_code', 'project_name', 'filament_name', 'quantity', 
                    'customer_name', 'total_price_display', 'profit_display', 'sale_date']
    list_filter = ['sale_date', 'filament_usage__filament__material', 'filament_usage__filament']
    search_fields = ['customer_name', 'customer_phone', 'filament_usage__project__model_name', 
                    'project_code', 'notes']
    readonly_fields = ['project_code', 'total_price', 'sale_date', 'unit_profit_display', 
                      'total_profit_display', 'sale_revenue_display']
    date_hierarchy = 'sale_date'
    
    fieldsets = (
        ('اطلاعات فروش', {
            'fields': ('filament_usage', 'project_code', 'quantity')
        }),
        ('مشتری', {
            'fields': ('customer_name', 'customer_phone')
        }),
        ('قیمت‌گذاری', {
            'fields': ('unit_price', 'packaging_cost', 'total_price')
        }),
        ('تحلیل', {
            'fields': ('sale_revenue_display', 'unit_profit_display', 'total_profit_display'),
            'classes': ('collapse',)
        }),
        ('یادداشت‌ها', {
            'fields': ('notes',),
    ...
    def project_name(self, obj):
        if obj.filament_usage and obj.filament_usage.project:
            return obj.filament_usage.project.model_name
        return "-"
    project_name.short_description = 'نام پروژه'
    
    def filament_name(self, obj):
        if obj.filament_usage and obj.filament_usage.filament:
            return f"{obj.filament_usage.filament.name} ({obj.filament_usage.filament.color})"
        return "-"
    filament_name.short_description = 'فیلامنت'
    ...
    def unit_profit_display(self, obj):
        return f"{obj.unit_profit:,.0f} تومان"
    unit_profit_display.short_description = 'سود واحد'
    
    def total_profit_display(self, obj):
        return f"{obj.total_profit:,.0f} تومان"
    total_profit_display.short_description = 'سود کل'
    
    def sale_revenue_display(self, obj):
        return f"{obj.sale_revenue:,.0f} تومان"
    sale_revenue_display.short_description = 'درآمد فروش'
